...
const SortableTaskItem: React.FC<{
  ...
  const { data: timeStatus, refetch: refetchTimeStatus } = useGetTimeTrackingStatusQuery(task.id, {
    pollingInterval: activeTracking?.taskId === task.id ? 1000 : 0, // Poll every second if active
  });
  ...
  const isActiveTimer = activeTracking?.taskId === task.id;
  const displayTime = isActiveTimer ? 
    activeTracking.currentDuration : 
    timeStatus?.totalTimeTracked || task.timeTracked || 0;
  ...
  React.useEffect(() => {
    if (isActiveTimer && timeStatus?.activeSession) {
      onTimerUpdate(task.id, timeStatus.activeSession.currentDuration);
    }
  }, [timeStatus?.activeSession?.currentDuration, isActiveTimer, task.id, onTimerUpdate]);

  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-4 p-4 mb-3 rounded-lg border dark:border-0 transition-all
        ${isDragging 
          ? 'shadow-lg bg-white/80 dark:bg-gray-800/50/80' 
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          title={timeStatus?.isActive && !isActiveTimer ? "Stop current timer first" : ""}
        >
          {isActiveTimer ? (
